...
class Interactor(nn.Layer):
    ...
    def forward(self, x):
        ...
        if self.modified:
            ...
            d = x_odd.multiply(paddle.exp(self.phi(x_even)))
            c = x_even.multiply(paddle.exp(self.psi(x_odd)))
...
class InteractorLevel(nn.Layer):
...
class LevelSCINet(nn.Layer):
    ...
    def forward(self, x):
        ...
        return paddle.transpose(x_even_update, perm=[0, 2, 1]), paddle.transpose(x_odd_update, perm=[0, 2, 1])  # even: B, T, D odd: B, T, D



class SCINet_Tree(nn.Layer):
    ...
    def zip_up_the_pants(self, even, odd):
        ...
        odd = paddle.transpose(odd, perm=[1, 0, 2])  # L, B, D
        ...
        return paddle.transpose(paddle.concat(_, 0), perm=[1, 0, 2])  # B, L, D
...
class SCINet(nn.Layer):
    def __init__(self, cfg):
        super(SCINet, self).__init__()

        self.input_dim = cfg['in_var']
        ...
        self.kernel_size = cfg['kernel']
        self.dropout = cfg['dropout']
        self.single_step_output_One = cfg['single_step_output_One']
        self.concat_len = cfg['concat_len']
        self.pe = cfg['positionalEcoding']
        self.RIN = cfg['RIN']
        self.num_decoder_layer = cfg['num_decoder_layer']
        ...
        if cfg['num_stacks'] == 2:  # we only implement two stacks at most.
        ...
        for m in self.named_sublayers():
            if isinstance(m, nn.Conv2D):
                ...
                m.bias.data.zero_()
        self.projection1 = nn.Conv1D(self.input_len, self.output_len, kernel_size=1, stride=1)
        self.div_projection = nn.LayerList()
        ...
        temp = paddle.arange(num_timescales, dtype=paddle.float32)
        inv_timescales = min_timescale * paddle.exp(
            paddle.arange(num_timescales, dtype=paddle.float32) *
            -log_timescale_increment)
        self.register_buffer('inv_timescales', inv_timescales)

        ### RIN Parameters ###
        if self.RIN:
            self.affine_weight = nn.Parameter(paddle.ones(1, 1, input_dim))
            self.affine_bias = nn.Parameter(paddle.zeros(1, 1, input_dim))
    
    def get_position_encoding(self, x):
        max_length = x.size()[1]
        position = paddle.arange(max_length, dtype=paddle.float32, device=x.device)  # tensor([0., 1., 2., 3., 4.], device='cuda:0')
        temp1 = position.unsqueeze(1)  # 5 1
        temp2 = self.inv_timescales.unsqueeze(0)  # 1 256
        scaled_time = position.unsqueeze(1) * self.inv_timescales.unsqueeze(0)  # 5 256
        signal = paddle.cat([paddle.sin(scaled_time), paddle.cos(scaled_time)], dim=1)  #[T, C]
    ...
    def forward(self, x):
        ...
        if self.RIN:
            ...
            x = x - means
            #var
            stdev = paddle.sqrt(paddle.var(x, dim=1, keepdim=True, unbiased=False) + 1e-5)
            x /= stdev
            # affine
        ...
        if self.num_decoder_layer == 1:
            ...
            for div_projection in self.div_projection:
                output = paddle.zeros(x.shape,dtype=x.dtype).cuda()
        ...
        if self.stacks == 1:
            ...
            MidOutPut = x
            if self.concat_len:
                x = paddle.cat((res1[:, -self.concat_len:,:], x), dim=1)
            else:
                x = paddle.cat((res1, x), dim=1)
...
def get_variable(x):
    x = Variable(x)
    return x.cuda() if paddle.cuda.is_available() else x
